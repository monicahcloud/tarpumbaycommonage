...
const attachmentSchema = z
  .object({
    target: z.enum(["APPLICATION", "COMMONER"]),
    applicationId: z.string().min(1).nullable().optional(), // ✅ optional now
    commonerId: z.string().min(1).nullable().optional(),
  ...
  .superRefine((val, ctx) => {
    if (val.target === "APPLICATION") {
      if (!val.applicationId) {
        ctx.addIssue({
          code: "custom",
          path: ["applicationId"],
          message: "applicationId is required for APPLICATION attachment.",
        });
      }
    }
    if (val.target === "COMMONER") {
      if (!val.commonerId) {
        ctx.addIssue({
          code: "custom",
          path: ["commonerId"],
          message: "commonerId is required for COMMONER attachment.",
        });
      }
    }
  });
...
export async function addAttachment(input: {
  ...
  // ✅ COMMONER-only path (no app required)
  if (target === "COMMONER") {
    const commoner = await prisma.commonerRegistration.findUnique({
      where: { id: commonerId! },
      select: { id: true, userId: true },
    ...
    // If applicationId is provided, enforce that it belongs to same user (safety)
    if (applicationId) {
      const app = await prisma.application.findUnique({
        where: { id: applicationId },
        select: { id: true, userId: true },
      ...
      if (!app) throw new Error("Application not found.");
      if (app.userId !== commoner.userId) {
        throw new Error("Application does not belong to this commoner.");
      }
    }
    ...
    // Admin audit log (optional; only attach applicationId if you have one)
    try {
      await prisma.adminEvent.create({
        data: {
          applicationId: applicationId ?? null, // ✅ ok even if null (must be nullable in schema)
          type: "ATTACHMENT_ADDED",
          actorClerkId: authz.userId,
          message: `Attachment added (COMMONER): ${rest.kind}`,
          meta: {
      ...
      // ignore if your AdminEvent model requires applicationId non-null
    }

    revalidatePath("/admin/commoners");
    revalidatePath(`/admin/commoners/${commoner.id}`);
    return;
  }

  // ✅ APPLICATION path (app required)
  const app = await prisma.application.findUnique({
    where: { id: applicationId! },
    select: { id: true },
  });
